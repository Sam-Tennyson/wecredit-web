/**
 * Zapcash SSO redirect service — polls `/api/forward` until utmLink is ready.
 */
...
import type { ZapcashSsoRedirectResponse } from '@/types/wecredit';

export interface ZapcashSsoRedirectResult {
  success: boolean;
  error?: string;
}

type ZapcashSsoPollAttemptResult =
  | { type: 'success'; utmLink: string }
  | { type: 'pending' }
  | { type: 'error'; error: string };

function parseZapcashSsoRedirectResponse(
  json: ZapcashSsoRedirectResponse,
): ZapcashSsoPollAttemptResult {
  const statusCodeNum =
  ...
  if (statusCodeNum === 404) {
    return {
      type: 'error',
      error: json.statusMessage ?? 'Eligible lead not found for provided mobile.',
    };
  }
...
async function fetchZapcashSsoRedirectAttempt(
  mobile: string,
  authorization: string | undefined,
  signal?: AbortSignal,
): Promise<ZapcashSsoPollAttemptResult> {
  const fetchOptions: RequestInit = {
    method: 'POST',
    headers: buildHeaders({ mobile, authorization }),
    body: JSON.stringify({
      endpoint: ENDPOINTS.FORWARD.ZAPCASH_SSO_REDIRECT,
  ...
  try {
    const json = JSON.parse(text) as ZapcashSsoRedirectResponse;
    return parseZapcashSsoRedirectResponse(json);
  } catch {
    return {
      type: 'error',
      error: 'Unexpected response from Zapcash redirect.',
    };
  }
}

export async function pollZapcashSsoRedirect(
  mobile: string,
  authorization?: string,
  signal?: AbortSignal,
): Promise<ZapcashSsoRedirectResult> {
  if (!mobile) {
    return {
      success: false,
  ...
  while (true) {
    ...
    let attempt: ZapcashSsoPollAttemptResult;
    try {
      attempt = await fetchZapcashSsoRedirectAttempt(mobile, authorization, signal);
    } catch (error) {
    ...
    if (elapsedMs >= FEDERALBANK_REDIRECT_MIN_POLL_MS && elapsedMs >= FEDERALBANK_REDIRECT_MAX_POLL_MS) {
      return {
        success: false,
        error: 'Zapcash redirect is taking longer than expected. Please try again.',
      };
    }
